  ...
  function fecha_db($fecha,$ini=false,$trunca=false)
    {
        ...
        if($trunca)$hora = '';
            ...
            if($this->formato=="d/m/Y"){
				...
				if($fecha1[1]<10) $fecha1[1] = '0'.$fecha1[1];
				if($fecha1[0]<10) $fecha1[0] = '0'.$fecha1[0];

               $final = $fecha1[2].'-'.$fecha1[1].'-'.$fecha1[0];

                }else{

               $final = $fecha1[2].'-'.$fecha1[0].'-'.$fecha1[1];
 ...
 ///////////// para fechas de tipo timestamp
  ...
  ////////////////// para fechas de tipo db datatime '0000-00-00 00:00:00'
   ...
   function datetime($datetime,$formato=false) {
	...
	/////cambia una fecha en formato db a formato dd/mm/yyyy
	public function switchfecha($fecha,$separador="/"){
		
		$nohora = explode(" ",trim($fecha));
		$fecha1 = explode("-",$nohora[0]);
		$fechan = $fecha1[2].$separador.$fecha1[1].$separador.$fecha1[0]." ".$nohora[1];
		return $fechan;
