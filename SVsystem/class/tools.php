 ...
 $j = 1;
  ...
  ///nota si se desea cerrar un popup coloque el texto: cerrar
      ...
      public function redirect($url,$parent=''){
				...
				if($url=="cerrar") $var = "window.close();"; else if($url=="actualizar") $var = "window.opener.location.reload(); window.close();"; else $var = "$parent"."location.replace('$url');";
...
////// abreviar necesita el texto a abreviar y el numero de caracteres a los que se va a reducir
 ...
 ///parametros: $dirname ruta del directorio, y si borra el directorio o no?
	...
	public function delete_directory($dir) {
		...
		//////////////////METODO DE COMPRESION DE UN DIRECTORIO NUEVO!!!!!!!
		/// primer parametro el origen del directorio que va a comprimir, y el segundo el nombre que va a tener el archivo comprimido
		///eg. Zip('/folder/to/compress/', './compressed.zip');

	 public function Zip($source, $destination, $baseDir) {
								   ...
								   if (is_dir($source) === true)  {
									  ...
									  $files = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($source), RecursiveIteratorIterator::SELF_FIRST);
										   foreach ($files as $file)  {
												  $file = realpath($file);
												   if (is_dir($file) === true)  {
															  $zip->addEmptyDir($baseDir.'/'.str_replace($source . '/', '', $file . '/'));
												 } else if (is_file($file) === true)  {
													 $zip->addFromString($baseDir.'/'.str_replace($source . '/', '', $file), file_get_contents($file));
												  }
												 }
											   }  else if (is_file($source) === true)  {
													   $zip->addFromString($baseDir.'/'.basename($source), file_get_contents($source));
												  }
												 }
												  return $zip->close();
											  }
