...
include("../../SVsystem/class/formulario.php");
	...
	if(isset($_POST['Submit'])){
		...
		$_POST['r-titulo'] = trim($_POST['titulo']);
			...
			$tool->update_data("r","-","artxcat",$_POST,"id = '{$_POST['id']}'");
		...
		$data = $tool->simple_db("select * from artxcat where id = '{$_REQUEST['id']}' ");
		if(!isset($_REQUEST['titulo'])) $_REQUEST['titulo'] = $data['titulo'];
		if(!isset($_REQUEST['fuente']))$pref = $data['fuente']; else $pref = $_REQUEST['fuente'];
...
<tr>
<td class="td-titulo-popup"> Editar  bloques de listados de art&iacute;culos en categor&iacute;a</td>
</tr>
<tr>
...
<tr>
  ...
  <td><span class="form-box">
    <select name="r-fuente" class="form-box" id="select" onChange="location.replace('<?=$PHP_SELF ?>?id=<?=$_REQUEST['id'] ?>&fuente='+this.value+'&titulo='+document.form1.titulo.value);">
...
<select name="r-cat" class="form-box" onChange="document.form1.catego.value = this.options[this.selectedIndex].text;">
 ...
 <?php $cats = $tool->estructura_db("select id,nombre,(select count(*) from ".$pref."_subcategoria where cat_id = c.id ) as subca from ".$pref."_categoria c order by orden");	
	 ...
	 ?>
	 <option value="1_<?php echo $cats[$c1]['id']?>" <?php if($data['cat']=="1_{$cats[$c1]['id']}") echo 'selected'?>><?php echo $cats[$c1]['nombre']?> </option>
	 ...
	 ?>
	 <option value="2_<?php echo $subcats[$c2]['id']?>" <?php if($data['cat']=="2_{$subcats[$c2]['id']}") echo 'selected'?>>&nbsp;&nbsp;<?php echo $subcats[$c2]['nombre']?>  </option>
     <?php
		   ...
		   $subsub = $tool->estructura_db("select id,nombre
			 ...
			 ?>
              <option value="3_<?php echo $subsub[$c3]['id']?>" <?php if($data['cat']=="3_{$subsub[$c3]['id']}") echo 'selected'?>>&nbsp;&nbsp;&nbsp;<?php echo $subsub[$c3]['nombre'] ?></option>
             <?
...
<td><input name="r-cantidad" type="text" class="form-box" id="r-cantidad" value="<?=$data['cantidad'] ?>" size="5"></td>
</tr>
<tr>
<td class="td-form-title">&iquest;Mostrar resumen?</td>
<td><select name="r-resumen" class="form-box" id="r-resumen">
  <option value="1" <?php if($data['resumen']=="1") echo 'selected'; ?>>Si 
  <option value="0" <?php if($data['resumen']=="0") echo 'selected'; ?>>No 
  </select>
  <input name="catego" type="hidden" id="catego" value="<?=$data['catego'] ?>">
...
<tr>
  ...
  <td><select name="r-ordeni" class="form-box" id="r-ordeni">
    <option value="asc" <?php if($data['ordeni']=="asc") echo 'selected'; ?> >Ascendente
      <option value="desc" <?php if($data['ordeni']=="desc") echo 'selected'; ?>>Descendente
      </select></td>
</tr>
<tr>
<td>&nbsp;</td>
