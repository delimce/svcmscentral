...
$form->autoconexion();
	...
	///////////////
			if(!empty($_FILES['archivo']['name'])){ ////si se sube una imagen
				...
				if($sesubio == false)$form->redirect("insert_sscategoria.php");
...
/*
  ...
  */
	...
	//////////////
					...
					$_POST['r-orden'] = $form->simple_db("select max(orden)+1 as total from cont_sub_subcategoria");
					$form->insert_data("r","-","cont_sub_subcategoria",$_POST);


					?>
                      <script language="JavaScript" type="text/JavaScript">
					  		window.opener.abrir_cat('<?=$_REQUEST['r-sub_id'] ?>','2');
							window.opener.abrir_cat('<?=$_REQUEST['r-sub_id'] ?>','2');
						   window.close();
					   </script>
                    <?
			...
			$estado = $form->simple_db("select acont_cat3 from preferencias ");
...
<head>
<title>Agregar sub sub categoria</title>
   ...
   <table width="100%" border="0" cellspacing="4" cellpadding="0">
    ...
    <tr style="display:none;">
     ...
     <select name="r_users" class="form-box" id="r_users">
       ...
       <input name="r-sub_id" type="hidden" id="r-sub_id" value="<?=$_REQUEST['id']?>"></td>
    </tr>
    <tr>
     <td>&nbsp;</td>
     <td>&nbsp;</td>
