...
////////paginacion
 ...
 $TOTAL = $tool->simple_db("select count(*) from cliente ");
 ...
 ///////////FILTRO
  ...
  if(isset($_REQUEST['band'])){ ///si viene de la pagina de busquedas
	 ...
	 if($_REQUEST['sede']!="") $filtrosede = " and sede LIKE '%{$_REQUEST['sede']}%' ";
	 ...
	 $_SESSION['QUERYX'].= $filtroname.$filtromail.$filtrorigen.$filtrocatego.$filtroempresa.$filtrotlf.$filtrocell.$filtroprofesion.$filtronivel.$filtroedo.$filtrocity.$filtrodir.$filtrosede.$filtropassword.$filtroo.$filtroe;
	 $_SESSION['QUERYY'].= $filtroname.$filtromail.$filtrorigen.$filtrocatego.$filtroempresa.$filtrotlf.$filtrocell.$filtroprofesion.$filtronivel.$filtroedo.$filtrocity.$filtrodir.$filtrosede.$filtropassword.$filtroo.$filtroe;
 }
...
&nbsp;  <a href="#" class="link-gris" onClick="popup('imprimir.php','imprime','595','1000');">Versi&oacute;n Imprimible <img src="../icon/icon-print.gif" width="16" height="14" border="0"></a>
...
<table width="100%" border="0" cellspacing="10" >
		...
		<tr class="bdc-tr-dato-container">
...
<td width="96%"><table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#E8ECEC">
				...
				<?php while($ii<$totalcampos){ ?>
				 <tr>
                   ...
                   <a href="javascript:;" onClick="popup('detallec.php?ide=<?=$row["id"] ?>','detalle','500','550');" class="instruccion"><?php echo $row[$campos1[$i]]?> <span>Click en el nombre para ver detalles de este usuario</span></a>
                   <?php if($row['ordenesn']>0){ ?><a href="../productos/ordenes.php?iduser=<?=$row["id"] ?>"  class="instruccion"><img border="0" src="../icon/icon-user-orden-nueva.gif" width="15" height="15"  align="absmiddle"> <font size="1" style="normal" >(<?php echo $row['ordenesn'] ?>)</font>  <span>Este usuario tiene órdenes de compra NUEVAS que usted no ha revisado!</span>      </a>


				   <?php }else if($row['ordenes']>0){ ?><a href="../productos/ordenes.php?iduser=<?=$row["id"] ?>"  class="instruccion"><img border="0" src="../icon/icon-user-ordenes.gif" width="15" height="15" align="absmiddle">  <font size="1"  style="normal"> (<?php echo $row['ordenes'] ?>)  </font>  <span>Órdenes de compra que ha realizado este usuario</span></a><?php } ?>&nbsp;

                   <?php if($row['pagosn']>0){ ?><a href="../pagos/main.php?iduser=<?=$row["id"] ?>" class="instruccion" ><img border="0" src="../icon/icon-user-pago-nuevo.gif" width="15" height="15" align="absmiddle">  <font size="1" style="normal">(<?php echo $row['pagosn'] ?>)</font> <span>Este usuario ha realizado pagos NUEVOS que  usted no ha revisado!</span></a>

                    <?php }else if($row['pagos']>0){ ?><a href="../pagos/main.php?iduser=<?=$row["id"] ?>" class="instruccion" ><img border="0" src="../icon/icon-user-pagos.gif" width="15" height="15" align="absmiddle"> <font size="1" style="normal">(<?php echo $row['pagos'] ?>) </font> <span>Pagos realizdos por este usuario</span></a><?php } ?>
                   </td>
					<?php
...
<div id="npaginacion"><?php paginas($TOTAL,$cuenta,$desde,"busquedac.php"); ?></div>
<!-- /termina nbloque -->
</div>
</form>
